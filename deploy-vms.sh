...
for ((i=1;i<=$vmnum; i++)); do
	...
	qm create $vmid --name $newname --memory $vmram --cores $corenum --ide0 local-lvm:$hdsize --net0 virtio,bridge=vmbr0 --ipconfig0 ip=192.168.100.100/24,gw=192.168.100.2 --ide1 /var/lib/vz/template/iso/ubuntu-22.04.1-live-server-amd64.iso,media=cdrom --boot c &
done
